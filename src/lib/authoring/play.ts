...
export type KeylessItem = Omit<Item, "answerKey" | "rationale">;

/**
 * The only item payload the play page sends to the browser (ADR 0003). The key and rationale stay
 * on the server until the score route returns them with a score. Works on a deep copy, so the
 * item it is given is never changed.
 */
