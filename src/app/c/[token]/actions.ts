 ...
 *
 * The request is counted exactly as sign-in counts one (#134's per-IP limit, then #139/#157's two
 * per-recipient counters), and the token is checked with the service role, which counts a wrong
 * token against the address. Once the token has resolved, every answer is the same `sent`: a new
 * address, an address that already has an account, one whose budget is spent. The account work and
 * the email happen in `after()`, once the answer has gone back, so not even the time taken can
 * say which it was.
 */
