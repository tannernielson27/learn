...
import { issueMessageId, type SaveResult } from "./EditorShell";
...
/**
 * The patient record: header, time points and sections of text, tables and vitals, beside the
 * record as the player shows it. Plain text and grids only, by the owner's decision for Sprint 5.
 * Save stores an unfinished record as a draft; the problems list says what it still needs.
 */
export function EhrEditor({ initialValues, onSave }: EhrEditorProps) {
  ...
  const fields: FieldHelpers = {
    idFor: (field) => `${ids}-field-${field.replace(/\./g, "-")}`,
    invalid: (field) => (issues.some((issue) => issue.field === field) ? true : undefined),
    describedBy: (field) =>
      issues.some((issue) => issue.field === field) ? issueMessageId(ids, field) : undefined,
  };
  ...
  const addTimePointId = `${ids}-add-time-point`;
  ...
  const change = (update: (current: EhrFormValues) => EhrFormValues) => setValues(update);

  function focusField(field: string) {
    const target =
      document.getElementById(fields.idFor(field)) ??
      // A vitals block's own problem ("needs at least one row") has no field of its own.
      document.getElementById(fields.idFor(`${field}.rows.0.label`)) ??
      document.getElementById(fields.idFor("patient.age"));
    target?.focus();
  }
  ...
  function move(index: number, delta: -1 | 1) {
    ...
    focusLater(moveButtonId(ids, target, same), moveButtonId(ids, target, other));
      ...
      >
        ...
        <fieldset className="flex flex-col gap-3">
          ...
          <p className="text-sm text-ink-2">
            A section charted at one time appears only then. With two or more times, the record
            shows a time selector.
          </p>
        ...
        <section aria-labelledby={`${ids}-sections`} className="flex flex-col gap-4">
          <h2 id={`${ids}-sections`} className="text-sm font-medium text-ink-1">
            Sections
          </h2>
