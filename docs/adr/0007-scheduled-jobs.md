...
- **The route is rate limited twice**, in the shared Postgres limiter like sign-in (`public.hit_rate_limit`, #234; in memory until then): wrong secrets per caller (so the secret cannot be guessed quickly) and authorised runs overall (so a leaked secret cannot drive the mailer flat out). The two budgets are separate, so a flood of bad guesses cannot lock out the real job.
...
- **A job that needs no secret may schedule itself.** The rate-limit sweep (#248) calls nothing outside the database, so there is nothing an owner must supply: its migration calls `private.schedule_rate_limit_sweep()`, which schedules it when pg_cron is on and answers `no_pg_cron` otherwise, and the owner calls it again after enabling pg_cron (§7.8 step 4). It is idempotent, updating the one job by name rather than adding a second.
