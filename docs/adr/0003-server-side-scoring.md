...
- **Amended 2026-09-20 (#146):** the gallery exception now names its boundary instead of trusting one. `/gallery/**` is **disabled on the production deployment and open everywhere else**. `src/app/gallery/layout.tsx` is the single layout every gallery route passes through; it `await`s `connection()` and then calls `galleryIsAvailable()` (`src/lib/gallery/availability.ts`), answering `notFound()` when `VERCEL_ENV` is `production`, because a 404 does not advertise that the surface exists. The signal is `VERCEL_ENV`, not `NODE_ENV` — Next sets `NODE_ENV` to `production` for preview builds too, so reading it would close the gallery on previews and in CI. Those stay open deliberately: previews, CI and localhost are where the gallery is actually used — sprint demos, the Playwright screenshot and axe run, a reviewer opening a route on a phone — and none of them has a real student on it, so gating them would force the screenshot run to authenticate for no security gain.
  - **The `connection()` call is part of the decision, not an implementation detail.** Without it nothing in the segment reaches for a request, so Next prerenders `/gallery/**` and bakes the gate's answer into the build artifact. The guarantee would then only be "this bundle was compiled with `VERCEL_ENV=production`" — and Vercel's **Promote to Production** ships an existing deployment without rebuilding it, so a preview artifact compiled with the gallery open could serve the production domain from the CDN, answer keys and all, with no build step left to catch it. The price is that `/gallery/**` is no longer statically generated. That is accepted: it is a debug surface, not a student path.
  - The guarantee is checked in three places. `src/app/gallery/layout.test.ts` **calls** the layout and asserts it refuses under `VERCEL_ENV=production` (and that it awaited `connection()` first, since the gate throwing proves the ordering) — a behavioural check, so a gate stranded in dead code or wrapped in a debug flag cannot pass it. `src/app/gallery/gate.test.ts` asserts every route answering on a `/gallery` URL renders inside that layout, and that any gallery route handler carries the gate itself, since layouts do not wrap those. `src/lib/gallery/availability.test.ts` covers the predicate.
