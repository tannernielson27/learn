...
- Shuffle: none in the #209 sense (the order is the answer). Separately, the **starting order** is always scrambled (#219), because authors type the steps in their correct order and the authored order is usually the key:
  ...
  - Seed: the item id by default (the author's play page, the gallery); `startingOrderSeed(sessionId, itemId)` in live sessions, both modes, so every phone in a room starts from the same order; `startingOrderSeed(attemptId, itemId)` for take-home attempts, whether or not the assignment shuffles, so a resume starts from the same order.
