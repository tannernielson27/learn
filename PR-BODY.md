...
- **Renderers load per type.** `registry.ts` wraps each renderer in `next/dynamic` with a literal `import()`, so each type gets its own chunk. A multiple choice page no longer loads the bowtie or dnd-kit. `RENDERERS[type]` still returns `{ Renderer, isComplete, explainScore }`.
...
- **Submit waits for the question, not only the answer.** An ordered response opens already arranged, so its answer is complete before the student sees it. With the renderer loading lazily, Submit was live over the placeholder (and over the load-failure message) and would have sent an order nobody chose. `RendererShownMarker` (`RendererShown.tsx`) sits beside the renderer inside the one Suspense boundary in `withLoadRecovery` and reports through a context, in a layout effect, when the renderer itself is on screen. `ItemPlayer` enables Submit only then. For this to hold, the `dynamic()` calls no longer pass `loading`: `next/dynamic` would otherwise wrap each renderer in a Suspense boundary of its own, and the marker would count the placeholder as the question. `withLoadRecovery`'s boundary shows the same `RendererLoading`, so nothing looks different.
...
- Added `RendererRecovery.player.test.tsx`. It plays a real `ItemPlayer` whose multiple choice module fails to load, and shows the stem and the Submit bar surviving; a second case shows an ordered response whose renderer failed keeps Submit off.
- Added `ItemPlayer.loading.test.tsx`: an ordered response's Submit stays off while its renderer's module is held back, and turns on once the question shows.
- `vitest.setup.ts` points `next/dynamic` at `next/dist/shared/lib/app-dynamic` for every UI test, which is what Next aliases it to in the App Router. Under Vitest it otherwise resolves to the Pages Router loader, which draws its own placeholder without suspending and swallows a failed load.
...
- Playwright, run locally because CI is unavailable: `pnpm build` and `pnpm start --port 3100` with `PLAYWRIGHT_BASE_URL`, pointed at the local Supabase stack, with `E2E_AUTH=1` and the local demo account, so one run covers both CI jobs (the gallery job's `pnpm e2e` and the `auth-e2e` job's spec list). Before the fix: 3 failed (`case-study.spec.ts` "the six steps run end to end", all three viewports), 175 passed, 26 skipped. `origin/main` (d4ae283) on the same setup: 178 passed, 26 skipped. After the fix: 178 passed, 26 skipped.
- The failure was the spec counting step 2's matrix checkboxes while the step's renderer was still the placeholder (a step change is the one place a new type's chunk loads after the page). `e2e/renderer.ts` adds `questionShown(page)`, which waits until no "Loading the question" placeholder remains, and the case study spec calls it on every step. Looking at it also turned up the Submit bug above.
- After the fix: `pnpm typecheck`, `pnpm lint`, `pnpm exec prettier --check src e2e vitest.setup.ts` pass; `pnpm vitest run` passes (194 files, 2277 tests).
