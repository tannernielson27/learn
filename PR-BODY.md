...
- Playwright against the local build (`PLAYWRIGHT_BASE_URL`): `feedback`, `motion`, `dragdrop`, `bowtie` and `ordered` pass. `case-study` "the six steps run end to end" first failed at step 2 on all three viewports, the same way on the #54 base. That was fixed on #54 in 9e7c5cb: the spec now waits for each step's renderer before counting its controls, and Submit no longer enables over a renderer that is still loading. This branch is rebased onto it.
- After the rebase onto 9e7c5cb: `pnpm typecheck` and `pnpm lint` pass; `pnpm vitest run src/components/question src/components/case-study src/components/live` passes (23 files, 188 tests, five runs in a row); `pnpm vitest run` passes (195 files, 2280 tests); `pnpm build` passes. The full Playwright suite, run against the local build and the local Supabase stack with `E2E_AUTH=1` (both CI jobs' specs), passes: 178 passed, 26 skipped, the same as `origin/main`.
- The rebase exposed a race in two of this branch's tests. #54 now runs every UI test with the App Router `next/dynamic` (`React.lazy`), and under it the reveal stays a plain transition, so the focus move onto the score panel (a passive effect) can land after the panel is first found. `ItemPlayer.server.test.tsx` and `ItemPlayer.transition.test.tsx` now wait for that focus with `waitFor`. Before this, they failed in about half of the runs after the rebase.
